<script lang="ts">
		...
		update(audioValue: number, deltaTime: number = 16): boolean {
			...
			switch (this.state) {
				case BubbleState.INFLATING:
					...
					this.radius = MIN_BUBBLE_SIZE + (this.baseRadius - MIN_BUBBLE_SIZE) * this.easeOutBack(this.inflationProgress);
					break;
	...
	class BeatDetector {
		...
		addEnergy(energy: number, currentTime: number): boolean {
			...
			const average = this.energyHistory.reduce((sum, val) => sum + val, 0) / this.energyHistory.length;
